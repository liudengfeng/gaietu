...
def view_error_counts_legend(session_state_key: str, idx=None):
    ...
    st.markdown(
        f"<div><span style='text-align: right; color: black; background-color: #FFFF00; margin-right: 5px;'>{n1}</span> <span>发音错误</span></div>",
        help="✨ 说得不正确的字词。",
...
def pronunciation_assessment_word_format(word_obj):
    ...
    if error_type == "Mispronunciation":
        result = f'<span style="color: black; background-color: #FFFF00; margin-right: 5px; text-decoration: underline; {underline_style}" title="{accuracy_score}">{word_obj.word}</span>'
    ...
    if word_obj.is_unexpected_break:
        ...
        result = (
            f'<span style="margin-right: 5px; {underline_style}">{word_obj.word}</span>'
        )
